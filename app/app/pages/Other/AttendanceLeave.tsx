...
export default function AttendanceLeave() {
  ...
  const fetchPendingCount = async () => {
    try {
        ...
        // ✅ response.data เป็นตัวเลขโดยตรง เช่น 5
    ...
    return (
        <View className="flex-1 bg-slate-50">
            <ScrollView contentContainerStyle={{ padding: 24 }} keyboardShouldPersistTaps="handled">
                <Text className="text-2xl font-bold text-gray-800 mb-6 text-center">บันทึกเวลา &amp; การลา</Text>

                <View className="bg-white rounded-2xl shadow-sm p-5 mb-8 border border-gray-100">
                    <Text className="text-xl font-bold text-gray-800 mb-4 text-center">บันทึกเวลาทำงาน</Text>
                    <View className="flex-row justify-between">
                        <TouchableOpacity onPress={handleCheckIn} className="flex-1 mr-2 items-center justify-center py-4 rounded-xl bg-indigo-50 active:bg-indigo-100 border border-indigo-200">
                            <MaterialCommunityIcons name="login" size={28} color="#4f46e5" />
                            <Text className="text-indigo-600 font-semibold text-base mt-2">เข้างาน</Text>
                            {checkInTime && <Text className="text-green-600 text-xs mt-1">เวลา: {checkInTime.toLocaleTimeString()}</Text>}
                        </TouchableOpacity>
                        <TouchableOpacity onPress={handleCheckOut} className="flex-1 ml-2 items-center justify-center py-4 rounded-xl bg-gray-50 active:bg-gray-100 border border-gray-200">
                            <MaterialCommunityIcons name="logout" size={28} color="#6b7280" />
                            <Text className="text-gray-600 font-semibold text-base mt-2">ออกงาน</Text>
                            {checkOutTime && <Text className="text-red-600 text-xs mt-1">เวลา: {checkOutTime.toLocaleTimeString()}</Text>}
                        </TouchableOpacity>
                    </View>
                </View>

                <View className="flex-row items-center justify-between border-b border-slate-100 pb-2 mb-2">
                    <Text className="text-base font-bold text-slate-800 ml-2">ลางาน : {pendingCount} ครั้ง</Text>
                    {/* ปุ่ม ประวัติ */}
                    <TouchableOpacity onPress={() => router.push("/pages/Profile/LeaveRequests")} className="flex-row items-center p-2 rounded-lg ">
